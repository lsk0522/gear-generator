...
- **하모닉 드라이브** — 모듈 / 플렉스스플라인 잇수(zf) / 서큘러스플라인
  잇수(zc)를 입력하면 서큘러스플라인 + 플렉스스플라인 + 웨이브제너레이터
  캠을 실시간 SVG로 미리보고 DXF로 내보낼 수 있습니다. 감속비·백래시·
  플렉스스플라인 변형률·맞물린 이 개수 같은 1차 성능 지표도 같이 표시됩니다.
  수식은 `web/core-math.js`에 정리되어 있으며, Yao et al., *"A Novel Cycloid
  Tooth Profile for Harmonic Drive with Fully Conjugate Features"*,
  Actuators 2025, 14(4), 187 의 사이클로이드 치형(Eq. 9–13)과 이빨각 α0
  절단(Sec. 3.3)을 그대로 구현하고, 서큘러스플라인 치형은 파형발생기 한
  바퀴 스윕에 대한 실제 수치 envelope(Eq. 14–15, 최대반경 래스터화)로
  계산합니다 — 근사식이 아닙니다. 자세한 내역은 파일 상단 주석과
...
| 폴더 | 생성 대상 | 제작 |
|---|---|---|
| `HarmonicDriveGenerator` | 하모닉 드라이브 3부품 (서큘러스플라인/플렉스스플라인/웨이브제너레이터 캠), 모듈·잇수(zf/zc) 입력, 실제 envelope 공액 치형 | 이 저장소 |
...
- **HarmonicDriveGenerator**: 플렉스스플라인은 단순 링 형태로 생성되며 실제
  컵(다이어프램+보스) 형상은 미포함. 웨이브제너레이터는 볼베어링 레이스가
  없는 순수 타원 캠입니다. Fusion 쪽 CAD 생성 정밀도(치형 정밀도/공액 계산
  해상도)는 웹 미리보기보다 훨씬 낮은 기본값을 씁니다 — 점 하나하나가 개별
  Fusion API 호출이라 스케치 곡선 수가 생성 속도를 좌우하기 때문입니다.
...
하모닉 드라이브 CTP 엔진(`web/core-math.js`, `fusion360_addin/HarmonicDriveGenerator/hd_math.py`)의
실제 수치 envelope·이빨각 절단·이뿌리 필렛 구성 방식은 Robrotics의
[Harmonic Maker](https://robrotics.web.app/gearmaker) (같은 논문을 구현한
공개 웹 도구)의 접근 방식을 참고해 독자적으로 재구현했습니다. 코드는 그대로
가져오지 않고, 논문 수식을 이 저장소의 방식대로 새로 작성했습니다.
